...
int xdp_udp_echo(struct xdp_md *ctx) {
    ...
    for (int i = 0; i < NUM_APP; i++) {
      bpf_user_met_direct_copy_tmp(ctx, payload_offset, port_array[i], STATS, sizeof(struct stats));
      payload_offset += sizeof(struct stats);
      bpf_user_met_direct_copy_tmp(ctx, payload_offset, port_array[i], STATS_STATE, sizeof(struct stats_state));
      payload_offset += sizeof(struct stats_state);
      bpf_user_met_direct_copy_tmp(ctx, payload_offset, port_array[i], SETTINGS, sizeof(struct settings));
      payload_offset += sizeof(struct settings);
      bpf_user_met_direct_copy_tmp(ctx, payload_offset, port_array[i], RUSAGE, sizeof(struct rusage));
      payload_offset += sizeof(struct rusage);
      bpf_user_met_direct_copy_tmp(ctx, payload_offset, port_array[i], THREAD_STATS, sizeof(struct thread_stats));
      payload_offset += sizeof(struct thread_stats);
      bpf_user_met_direct_copy_tmp(ctx, payload_offset, port_array[i], SLAB_STATS, sizeof(struct slab_stats));
      payload_offset += sizeof(struct slab_stats);
      bpf_user_met_direct_copy_tmp(ctx, payload_offset, port_array[i], TOTALS, sizeof(itemstats_t));
